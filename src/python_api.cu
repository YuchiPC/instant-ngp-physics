...
void Testbed::Nerf::Training::set_image(int frame_idx, pybind11::array_t<float> img, pybind11::array_t<float> depth_img, float depth_scale) {
	...
	dataset.set_training_image(
		frame_idx,
		{(int)img_buf.shape[1], (int)img_buf.shape[0]},
...
void Testbed::override_sdf_training_data(py::array_t<float> points, py::array_t<float> distances) {
	...
	CUDA_CHECK_THROW(cudaMemcpyAsync(
		m_sdf.training.distances.data(), distances_cpu.data(), distances_buf.shape[0] * sizeof(float), cudaMemcpyHostToDevice, m_stream.get()
	));
...
pybind11::dict Testbed::compute_marching_cubes_mesh(ivec3 res3d, BoundingBox aabb, float thresh) {
	...
	CUDA_CHECK_THROW(cudaMemcpy(
		cpunormals.request().ptr, m_mesh.vert_normals.data(), m_mesh.vert_normals.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost
	));
	CUDA_CHECK_THROW(
		cudaMemcpy(cpucolors.request().ptr, m_mesh.vert_colors.data(), m_mesh.vert_colors.size() * 3 * sizeof(float), cudaMemcpyDeviceToHost)
	);
...
PYBIND11_MODULE(pyngp, m) {
	...
	testbed.def(py::init<ETestbedMode>(), py::arg("mode") = ETestbedMode::None)
		...
		.def(
			"predict_view_camera",
			&Testbed::predict_view_camera,
			"Outputs the predicted camera matrix of a given view after a given elapsed time (in seconds)."
		)
		.def_property("camera_prediction_mode", &Testbed::camera_prediction_mode, &Testbed::set_camera_prediction_mode)
#ifdef NGP_GUI
		...
		.def(
			...
			&Testbed::reload_network_from_json,
		...
		.def(
			...
			py::arg("path"),
			py::arg("include_optimizer_state") = false,
			py::arg("compress") = true,
			"Save a snapshot of the currently trained model. Optionally compressed (only when saving '.ingp' files)."
		)
		...
		.def_property("jit_fusion", &Testbed::jit_fusion, &Testbed::set_jit_fusion)
		.def(
		...
		.def("compute_image_mse", &Testbed::compute_image_mse, py::arg("quantize") = false)
		...
		.def_property(
			"display_gui",
			[](py::object& obj) { return obj.cast<Testbed&>().m_imgui.show; },
			[](const py::object& obj, bool value) { obj.cast<Testbed&>().m_imgui.show = value; }
		)
	...
	nerf.def_readonly("training", &Testbed::Nerf::training)
		...
		.def_readwrite(
			"rendering_extra_dims_from_training_view",
			&Testbed::Nerf::rendering_extra_dims_from_training_view,
			"If non-negative, indicates the training view from which the extra dims are used. If -1, uses the values previously set by `set_rendering_extra_dims`."
		)
		.def(
			"find_closest_training_view",
			&Testbed::Nerf::find_closest_training_view,
			"Obtain the training view that is closest to the current camera."
		)
		.def(
			"set_rendering_extra_dims_from_training_view",
			&Testbed::Nerf::set_rendering_extra_dims_from_training_view,
			"Set the extra dims that are used for rendering to those that were trained for a given training view."
		)
	...
	nerfdataset.def_readonly("metadata", &NerfDataset::metadata)
	...
	py::class_<Testbed::Nerf::Training>(nerf, "Training")
		...
		.def(
			"get_extra_dims",
			&Testbed::Nerf::Training::get_extra_dims_cpu,
			"Get the extra dims (including trained latent code) for a specified training view."
		)
		.def(
		...
		.def(
			...
			py::arg("frame_idx"),
			"return the 3x4 transformation matrix of given training frame"
		)
		.def(
			"set_image",
	...
	image.def_readonly("training", &Testbed::Image::training).def_readwrite("random_mode", &Testbed::Image::random_mode);
	...
	py::class_<CameraKeyframe>(m, "CameraKeyframe")
		.def(py::init<>())
		.def(
			py::init<const quat&, const vec3&, float, float, float, float, float>(),
			py::arg("r"),
			py::arg("t"),
			py::arg("slice"),
			py::arg("scale"),
		...
		.def(
			py::init<const mat4x3&, float, float, float, float, float>(),
			py::arg("m"),
			py::arg("slice"),
			py::arg("scale"),
			py::arg("fov"),
	...
	py::class_<CameraPath::RenderSettings>(m, "CameraPathRenderSettings")
		...
		.def_readwrite("duration_seconds", &CameraPath::RenderSettings::duration_seconds)
	...
	py::class_<CameraPath>(m, "CameraPath")
		...
		.def_readwrite("duration_seconds", &CameraPath::duration_seconds)
		.def_readwrite("camera_prediction_duration_seconds", &CameraPath::camera_prediction_duration_seconds)
		.def_readwrite("camera_prediction_dt", &CameraPath::camera_prediction_dt)
